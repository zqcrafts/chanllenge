...
class TBLRAnchorCoder(BaseBBoxCoder):
    """TBLR BBox coder.
    ...
    def __init__(self, normalizer=4.0, clip_border=True, normalize_by_wh=False):
        super(TBLRAnchorCoder, self).__init__()
...
def bboxes2tblr(priors, gts, normalizer=4.0, normalize_by_wh=True):
    ...
    top = priors[:, 1].unsqueeze(1) - ymin
    bottom = ymax - priors[:, 3].unsqueeze(1)
    left = priors[:, 0].unsqueeze(1) - xmin
    right = xmax - priors[:, 2].unsqueeze(1)
    loc = priors.cat((top, bottom, left, right), dim=1)
    if normalize_by_wh:
...
def tblr2bboxes(priors,
    ...
    xmin = priors[:, 0].unsqueeze(1) - left
    xmax = priors[:, 2].unsqueeze(1) + right
    ymin = priors[:, 1].unsqueeze(1) - top
    ymax = priors[:, 3].unsqueeze(1) + bottom
