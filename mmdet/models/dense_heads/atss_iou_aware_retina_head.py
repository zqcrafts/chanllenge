...
class ATSSIoUAwareRetinaHead(AnchorHead):
    ...
    def __init__(self,
        ...
        super(ATSSIoUAwareRetinaHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, iou_pred, anchors in zip(cls_score_list,
            ...
            if self.alpha is None:
                scores *= iou_pred    # score
            elif isinstance(self.alpha, float):  # alpha = 0.4
                scores = torch.pow(scores, 2 * self.alpha) * torch.pow(
                    iou_pred, 2 * (1 - self.alpha))  # pow(x,y) x的y次方
            else:
